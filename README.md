...
7. **Liên hệ** — địa chỉ vườn (link Google Maps) và 2 nút hành động nhanh: **Chat qua Zalo** và **Gọi điện** trực tiếp (không còn nhúng bản đồ Google Maps dạng iframe).
8. **Góp ý** — form Google Forms nhúng (iframe, cao 650px) để khách góp ý ẩn danh.
9. **Footer**.
10. **Google Analytics (gtag.js)** — chỉ tải trên domain production (`tony-jpham.github.io/vuon-va-nang/`), không chạy khi test local. Bắn các event tuỳ chỉnh: `view_product_image` (mở ảnh sản phẩm), `select_product_contact` (bấm liên hệ trên trang chi tiết), `share_product_link` (sao chép link sản phẩm), `select_contact` (bấm Zalo/gọi điện ở mục Liên hệ trang chủ).
11. **Favicon** — biểu tượng lá 🌿 nhúng dạng SVG data URI (không cần file ảnh riêng).
